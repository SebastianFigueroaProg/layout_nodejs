...
//Arrays Call T
for (let i = 1; i < 16; i++) {    
    divCallT.push(document.querySelector(`.elementT${i}`))    
}
for (let i = 1; i < 16; i++) {    
    arrayDomBoxT.push(document.querySelector(`.elementT${i}`).textContent);       
}
...
// Peticiones hacia el Backend Call T
fetch('/data/callz')
.then( res => res.json())
.then(resultado =>{
    estadoBoxCallT(resultado)    
    tituloBoxT(resultado)
    ocupacionCallT(resultado)  
})
//CALL L
...
//CALL T
//Funcion para pintar la situacion de los BOX del Call T
const estadoBoxCallT =(data) =>{
    ...
    //Bucle donde se compara la coicidencia de BOX para pintar correctamente dependiendo el estado del mismo
    for (let i = 0; i < arrayDomBoxT.length; i++) {
        
        if (arrayBoxDataTm[i] == arrayDomBoxT[i] && arrayBoxDataTt[i] == arrayDomBoxT[i]) {
            if(arrayEstadoTt[i] === 'Ocupado' && arrayEstadoTm[i] === 'Ocupado'){
                divCallT[i].style.backgroundColor=ocupadoTlT;
            }else if(arrayEstadoTt[i] === 'Ocupado' && arrayEstadoTm[i] != 'Ocupado'){
                divCallT[i].style.backgroundColor=ocupadoTt;
            }else if(arrayEstadoTt[i] != 'Ocupado' && arrayEstadoTm[i] === 'Ocupado'){
                divCallT[i].style.backgroundColor=ocupadoTm;
            }else if(arrayEstadoTt[i] === 'Disponible' && arrayEstadoTm[i] === 'Disponible'){
                divCallT[i].style.backgroundColor=disponible;
            }else if(arrayEstadoTt[i] === 'Disponible sin PC' && arrayEstadoTm[i] === 'Disponible sin PC'){
                divCallT[i].style.backgroundColor=DisponibleSinPC;
            }else if (arrayEstadoTt[i] === 'Por Ordenanza' && arrayEstadoTm[i] === 'Por Ordenanza'){
                divCallT[i].style.backgroundColor=porOrdenanza;                        
                    }            
        }else{
            console.log("error en pintar DIV")
        }
...
//Funcion para poner titulo a los elemento para mostrar datos del Box posicionado Call T
const tituloBoxT = (data) =>{
    ...
    for (let  i= 0; i < arrayDomBoxT.length ; i++){
        ...
        if (arrayBoxDataTm[i] == arrayDomBoxT[i] && arrayBoxDataTt[i] == arrayDomBoxT[i]) {
            ...
            divCallT[i].setAttribute('title',`${arrayBoxDataTm[i]}
            Turno M.: 
            Agente: ${nombreTm[i]}
            Ingreso: ${ingresoTm[i]}--Egreso:${egresoTm[i]}
            Turno T.:
...
// Funcion para mostrar Ocupacion del Call T
const ocupacionCallT = (data) =>{

    const p1 = document.getElementById('p22');
    const p2 = document.getElementById('p23');
    const p3 = document.getElementById('p24');
    ...
    // Variables para el Total Call T    
    let suma = ocupacion+disponibilidad+sinUtilizacion; 
    let porOcu = ocupacion * 100 / suma; 
    let resul = porOcu.toFixed(2);
    let porDis = disponibilidad * 100 / suma;
    ...
    // Modifica etiqueta pra mostras Ocupacion Call T
    p1.innerHTML = ` TM: ${resulTm}% - TT: ${resulTt}% - Tot: ${resul}%`
    p2.innerHTML = ` TM: ${resDisTm}% - TT: ${resDisTt}% - Tot: ${resDis}%`
    p3.innerHTML = ` TM: ${resSutiTm}% - TT: ${resSutiTt}% - Tot: ${resSuti}%`  
